...
#include <opencv2/core.hpp>         //OPENCV 的头文件
...
void print_depth(astra_depthframe_t depthFrame)
{
    ...
    //-----------------------------在程序运行的目录下，创建一个文件夹，把图像存入文件夹中------------------------------------------------
    FramesSamples--;//每次采集完1帧数据，FramesSamples就自减1，FramesSamples计数值是为了防止无限保存图像用的
    ...
    leftup    = depthData[width * 8 - 20];                      //测4个角的深度值，因为相机深度传感器可能在边缘有比较多坏点，所以要取靠中间一些的点位
