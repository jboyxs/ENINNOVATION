...
N_FRAMES = 5  # 缓存帧数，用于计算平均值

def mousecallback(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDBLCLK:
        print(f"Depth at ({y}, {x}): {dpt[y, x]} mm")

def weighted_average(positions):
    """计算缓存队列中的加权平均位置"""
    weights = np.linspace(1, 0.5, len(positions))  # 为最近的帧赋予更高权重
    positions = np.array(positions)
    return np.average(positions, axis=0, weights=weights).astype(int)
...
if __name__ == "__main__":
    ...
    # 使用队列存储最近 N 帧的小球位置
    ball_positions = deque(maxlen=N_FRAMES)
    ...
    while True:
        ...
        if tracking and tracker is not None:
            ...
            if success:
                ...
                detected = True  # 标记为检测到小球
                ball_positions.append([p1[0] + ball_bbox[2] // 2, p1[1] + ball_bbox[3] // 2])
        ...
        if not detected and len(ball_positions) > 0:
            # 如果当前帧未检测到小球，但历史帧有数据，则使用加权平均的位置信息
            avg_position = weighted_average(ball_positions)
            cv2.circle(color_frame, tuple(avg_position), 5, (255, 0, 0), -1)
            print(f"Using averaged position: {avg_position}")
